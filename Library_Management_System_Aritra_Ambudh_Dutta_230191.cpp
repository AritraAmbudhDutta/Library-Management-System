...
private:
    ...
    string borrowedBy;
    string reservedBy; // NEW: Track who has reserved this book
    time_t borrowDate;

public:
    // Public constructor to initialize Book object
    Book(const string &title, const string &author, const string &publisher, const string &year, const string &ISBN, const string &status, const string &borrowedBy, time_t borrowDate, const string &reservedBy = "None")
        : title(title), author(author), publisher(publisher), year(year), ISBN(ISBN), status(status), borrowedBy(borrowedBy), reservedBy(reservedBy), borrowDate(borrowDate) {}
    ...
    {
        if (reservedBy != "None") {
            // If reserved, mark as Reserved instead of Available
            status = "Reserved";
        } else {
            // Otherwise mark as Available as before
            status = "Available";
        }
        borrowedBy = "None";
        borrowDate = 0;
    ...
    bool reserveBook(const string &userID) {
        // Can only reserve a book that is currently borrowed
        if (status == "Borrowed" && reservedBy == "None") {
            reservedBy = userID;
            return true;
        }
        return false;
    ...
    bool borrowReservedBook(const string &userID) {
        if (status == "Reserved" && reservedBy == userID) {
            status = "Borrowed";
            borrowedBy = userID;
            reservedBy = "None";
            borrowDate = time(nullptr);
    ...
    static vector<Book> readBookData(const string &filename)
    {
        ...
        while (getline(file, line))
        {
            ...
            string title, author, publisher, year, ISBN, status, borrowedBy, reservedBy;
            time_t borrowDate;
            ...
            Book book(title, author, publisher, year, ISBN, status, borrowedBy, borrowDate, reservedBy);
    ...
    static void writeBookData(const vector<Book> &books, const string &filename)
    {
        ...
        for (size_t i = 0; i < books.size(); ++i)
        {
            ...
            file << book.getTitle() << ","
                 ...
                 << book.getBorrowedBy() << ","
                 << book.getBorrowDate() << ","
                 << book.getReservedBy();  // The reservedBy field is included
            if (i != books.size() - 1)
            {
                file << "\n";
            }
...
public:
    ...
    void calculateFine(bool isStudent)
    {
        ...
        {
            int daysBorrowed = (now - entry.second) / (24*60*60);
    ...
    void showBorrowedBooks() const
    {
        ...
        {
            int daysBorrowed = (now - borrowDates.at(bookISBN)) / (24*60*60);
...
public:
    ...
    int calculateStudentFine(const string &studentID) const
    {
        ...
        for (const auto &book : books)
        {
            ...
            {
                int daysBorrowed = (now - book.getBorrowDate()) / (24*60*60);
                if (daysBorrowed > 15)
                {
    ...
    void updateBorrowRecord(const string &userID, Book *book, int finePaid)
    {
        ...
        for (auto &record : borrowHistory)
        {
            ...
            {
                int daysKept = (now - record.getBorrowDate()) / (24*60*60); // Using test divisor
    ...
    vector<Book*> findBooksByStatus(const string &status) {
        vector<Book*> result;
        for (auto &book : books) {
    ...
    void listReservedBooks(const string &userID) {
        cout << "\n=== BOOKS RESERVED BY USER: " << userID << " ===\n";
        bool found = false;
        ...
        for (auto &book : books) {
            if (book.getReservedBy() == userID) {
                found = true;
                book.displayDetails();
            } else if (book.getStatus() == "Reserved" && book.getReservedBy() == userID) {
                found = true;
                book.displayDetails();
            }
        }
        ...
        if (!found) {
            cout << "You have no reserved books.\n";
        }
    }

    void displayAllReservedBooks() const {
        cout << "\n=== ALL RESERVED BOOKS ===\n";
        bool foundAny = false;
        
        for (const auto &book : books) {
            if (book.getStatus() == "Reserved" || book.getReservedBy() != "None") {
                foundAny = true;
                book.displayDetails();
            }
        }
    ...
    void displayAvailableBooks() const {
        ...
        if (!foundAny) {
            cout << "No books are currently available for borrowing.\n";
        }
    }
};
        ...
        while (true)
        {
            ...
            if (userType == 1)
            {
                ...
                while (true)
                {
                    ...
                    if (choice == 10)  // Changed from 9 to 10
                        break;
                    ...
                    case 6: // Reserve a Book
                    {
                        ...
                        if (!foundAny) {
                            cout << "No books available for reservation at this time.\n";
                            break;
                        }
                        ...
                        // NEW CHECK: Prevent reserving your own borrowed book
                        if (book->getBorrowedBy() == student->getID()) {
                            cout << "You cannot reserve a book that you have already borrowed.\n";
                            break;
                        }
                        
                        if (book->getStatus() != "Borrowed") {
                            cout << "Only books that are currently borrowed can be reserved.\n";
                            break;
                        }
                        
                        if (book->getReservedBy() != "None") {
                            cout << "This book is already reserved by someone else.\n";
                            break;
                        }
                        
                        if (book->reserveBook(student->getID())) {
                            library.saveData();  // This is already there
                            cout << "Book reserved successfully! You will be notified when it's available.\n";
                        } else {
                            cout << "Could not reserve the book.\n";
                        }
                        break;
                    }
                    
                    case 7: // Borrow Reserved Book
                    {
                        // Show only books reserved by this student
                        cout << "\n=== BOOKS RESERVED FOR YOU ===\n";
                        bool foundAny = false;
                        for (auto &book : library.getBooks()) {
                            if (book.getStatus() == "Reserved" && book.getReservedBy() == student->getID()) {
                                foundAny = true;
                                book.displayDetails();
                            }
                        }
                        ...
                        if (!foundAny) {
                            cout << "You have no books reserved and available for borrowing.\n";
                            break;
                        }
                        
                        string ISBN;
                        cout << "Enter ISBN of the reserved book to borrow: ";
                        cin >> ISBN;
                        ...
                        if (book->getStatus() != "Reserved" || book->getReservedBy() != student->getID()) {
                            cout << "This book is not reserved for you.\n";
                            break;
                        }
                        ...
                        if (book->borrowReservedBook(student->getID())) {
                            student->borrowBook();
                            library.addBorrowRecord(student->getID(), book);
                            library.saveData();
                    ...
                    case 8: // View Reserved Books
                    {
                        library.listReservedBooks(student->getID());
                        break;
                    }
                    
                    case 9: // View Available Books
                    {
            ...
            else if (userType == 2)
            {
                ...
                while (true)
                {
                    ...
                    if (choice == 9) // Changed from 8 to 9
                        break;
                    ...
                    case 1: // Borrow Book
                    {
                        ...
                        for (auto &b : library.getBooks())
                        {
                            ...
                            {
                                time_t now = time(nullptr);
                                double daysBorrowed = difftime(now, b.getBorrowDate()) / (24*60*60);
                    ...
                    case 5: // Reserve a Book (similar to student case 6)
                    {
                        cout << "\n=== CURRENTLY BORROWED BOOKS AVAILABLE FOR RESERVATION ===\n";
                        bool foundAny = false;
                        for (auto &book : library.getBooks()) {
                        ...
                        // NEW CHECK: Prevent reserving your own borrowed book
                        if (book->getBorrowedBy() == faculty->getID()) {
                            cout << "You cannot reserve a book that you have already borrowed.\n";
                            break;
                        }
                        ...
                        if (book->reserveBook(faculty->getID())) {
                            library.saveData();  // This is already there
                            cout << "Book reserved successfully! You will be notified when it's available.\n";
                        } else {
                            cout << "Could not reserve the book.\n";
                    ...
                    case 6: // Borrow Reserved Book (similar to student case 7 but with faculty limits)
                    {
                        // Show only books reserved by this faculty
                        cout << "\n=== BOOKS RESERVED FOR YOU ===\n";
                        bool foundAny = false;
                        for (auto &book : library.getBooks()) {
                            if (book.getStatus() == "Reserved" && book.getReservedBy() == faculty->getID()) {
                                foundAny = true;
                                book.displayDetails();
                            }
                        }
                        ...
                        if (book->borrowReservedBook(faculty->getID())) {
                            faculty->borrowBook();
                            library.addBorrowRecord(faculty->getID(), book);
                            library.saveData();
                            cout << "Reserved book borrowed successfully!\n";
                    ...
                    case 7: // View Reserved Books
                    {
                        library.listReservedBooks(faculty->getID());
                        break;
                    }
                    
                    case 8: // View Available Books
                    {
                        library.displayAvailableBooks();
                        break;
                    }
            ...
            else if (userType == 3)
            {
                ...
                while (true)
                {
                    ...
                    if (choice == 12) // Changed from 11 to 12
                        break;
                    ...
                    case 11: // View All Reserved Books
                    {
